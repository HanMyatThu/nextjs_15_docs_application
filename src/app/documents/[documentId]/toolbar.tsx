...
const FontSizeButton = () => {
  ...
  return (
    <div className="flex items-center gap-x-0.5">
      <button
        onClick={decrement}
        className="size-7 flex flex-col gap-y-0 items-center justify-center rounded-sm hover:bg-neutral-200/80 px-1.5 overflow-hidden text-sm"
      >
...
const AlignButton = () => {
  ...
  const alignValue = editor?.getAttributes("textAligns").value || "left";

  const { icon: CurrentIcon } = textAligns.find(
    (align) => align.value === alignValue
  ) || { icon: AlignLeftIcon };
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="p-1 flex flex-col gap-1">
        {textAligns.map(({ label, value, icon: Icon }) => (
          <button
            key={label}
            onClick={() => editor?.chain().focus().setTextAlign(value).run()}
            className={cn(
              "flex items-center gap-x-2 px-2 py-1 rounded-sm hover:bg-neutral-200/80",
              editor?.isActive({ textAligns: value }) && "bg-neutral-200/80"
            )}
          >
            <Icon className="size-4" />
            <span className="text-sm">{label}</span>
